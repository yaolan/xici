...
public class BabushkaText extends TextView {
	...
	/**
	 * Adds a Piece at this specific location. The underlying data structure is
	 * a {@link List}, so expect the same type of behaviour.
	 *
	 * @param aPiece
	 *            the Piece to add.
	 * @param location
	...
	/**
	 * Replaces the Piece at the specified location with this new Piece. The
	 * underlying data structure is a {@link List}, so expect the same
	 * type of behaviour.
	 *
	 * @param newPiece
	 *            the Piece to insert.
	...
	/**
	 * Removes the Piece at this specified location. The underlying data
	 * structure is a {@link List}, so expect the same type of
	 * behaviour.
	 *
	 * @param location
	 *            the index of the Piece to remove
	 */
	...
	/**
	 * Call this method when you're done adding
	 *
	 * You MUST also call this method whenever you make a modification to the
	 * text of a Piece that has already been displayed.
	 */
	public void display() {
		...
		for (Piece aPiece : mPieces) {
			applySpannablesTo(aPiece, finalString, cursor,
	...
	private void applySpannablesTo(Piece aPiece, SpannableString finalString,
			int start, int end) {
		...
		// relative text size
		finalString.setSpan(new RelativeSizeSpan(aPiece.textSizeRelative),
				start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

		// text color
		finalString.setSpan(new ForegroundColorSpan(aPiece.textColor), start,
				end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

		// background color
		if (aPiece.backgroundColor != -1) {
			finalString.setSpan(
					new BackgroundColorSpan(aPiece.backgroundColor), start,
					end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
	}
	...
	/**
	 * A Piece represents a part of the text that you want to style. Say for
	 * example you want this BabushkaText to display "Hello World" such that
	 * "Hello" is displayed in Bold and "World" is displayed in Italics. Since
	 * these have different styles, they are both separate Pieces.
	 *
	 * You create a Piece by using it's
	 *
	 */
	public static class Piece {
		...
		/**
		 * Sets the text of this Piece. If you're creating a new Piece, you
		 * should do so using it's
		 *
		 * Use this method if you want to modify the text of an existing Piece
		 * that is already displayed. After doing so, you MUST call
		 * {@code display()} for the changes to show up.
		 *
		 * @param text
		 *            the text to display
		 */
		...
		/**
		 * Sets the text color of this Piece. If you're creating a new Piece,
		 * you should do so using it's
		 *
		 * Use this method if you want to change the text color of an existing
		 * Piece that is already displayed. After doing so, you MUST call
		 * {@code display()} for the changes to show up.
		 *
		 * @param
		 *            of text (it is NOT android Color resources ID, use
		 *            getResources().getColor(R.color.colorId) for it)
		 */
